...
export function useTimer(options: TimerOptions = {}, callback?: (overdueCallCount: number) => void) {
  ...
  const [StartedRef, setStartedRef] = useState(() => {
    const savedStartedRef = localStorage.getItem("beepr:startedref")
    if (savedStartedRef) {
      return JSON.parse(savedStartedRef)
    } else { return false }
  })
  useEffect(() => {
    localStorage.setItem("beepr:startedref", JSON.stringify(StartedRef))
  }, [StartedRef])
  ...
  const [PauseTimeRef, setPauseTimeRef] = useState(() => {
    const savedPauseTimeRef = localStorage.getItem("beepr:pausetimeref")
    if (savedPauseTimeRef) {
      return JSON.parse(savedPauseTimeRef)
    } else { return never }
  })
  useEffect(() => {
    localStorage.setItem("beepr:pausetimeref", JSON.stringify(PauseTimeRef))
  }, [PauseTimeRef])
  ...
  const [StartTimeRef, setStarTimeRef] = useState(() => {
    const savedStartTimeRef = localStorage.getItem("beepr:starttimeref")
    if (savedStartTimeRef) {
      return JSON.parse(savedStartTimeRef)
    } else { return never }
  })
  useEffect(() => {
    localStorage.setItem("beepr:starttimeref", JSON.stringify(StartTimeRef))
  }, [StartTimeRef])
  ...
  const [TotalElapsedPauseTimeRef, setTotalElapsedPauseTimeRef] = useState(() => {
    const savedTotalElapsedPauseTimeRef = localStorage.getItem("beepr:totalelapsedpausetimeref")
    if (savedTotalElapsedPauseTimeRef) {
      return JSON.parse(savedTotalElapsedPauseTimeRef)
    } else { return 0 }
  })
  useEffect(() => {
    localStorage.setItem("beepr:totalelapsedpausetimeref", JSON.stringify(TotalElapsedPauseTimeRef))
